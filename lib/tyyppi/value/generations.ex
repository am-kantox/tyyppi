if Code.ensure_loaded?(StreamData) do
  ...
  defmodule Tyyppi.Value.Generations do
    ...
    def any, do: Stream.cycle([:ok, 42, "foo"])

    def atom(kind \\ :alphanumeric) when is_atom(kind),
      do: Stream.cycle([:ok, :error])

    def string(_ \\ []), do: Stream.cycle(["foo", "bar", "baz"])
    ...
    def timeout(_top \\ 42), do: Stream.cycle([42, :infinity])

    def pid, do: Stream.cycle([Value.pid(0, 42, 0)])

    def mfa(_opts \\ []), do: Stream.cycle([Value.mfa(value: {Integer, :to_string, 2})])

    def mod_arg(_opts \\ []), do: Stream.cycle([Value.mod_arg(value: {Integer, [42]})])
